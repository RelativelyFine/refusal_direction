...
def chat(model_path, vectors_ablated):
    ...
    while prompt:
        print("\nGood Bot (:D):",
              model_base.answer_prompt(prompt),
              max_new_tokens=1024)  # Non ablated response.
        print("\nEvil Bot (>:O):",
              model_base.answer_prompt(prompt,
                                       fwd_pre_hooks=ablation_fwd_pre_hooks1,
                                       fwd_hooks=ablation_fwd_hooks1),
              max_new_tokens=1024)  # Singly ablated response.
        print("\nSUPER Evil Bot (>:X):",
              model_base.answer_prompt(prompt,
                                       fwd_pre_hooks=sum_ablation_fwd_pre_hooks,
                                       fwd_hooks=sum_ablation_fwd_hooks),
              max_new_tokens=1024)  # Fully ablated response.
        prompt = input("\nYou: ")
